        ...
        <table class="w-full text-sm text-left min-w-[1000px]">
            <thead class="text-xs text-slate-500 uppercase bg-slate-50/50 border-y border-slate-200">
                <tr>
                        ...
                        <th class="px-6 py-4 text-left font-bold text-slate-800">No. PO</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td class="px-6 py-4 text-slate-600 font-medium">{{ $p->no_po ?? '-' }}</td>
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onclick="editProyek({{ $p->id }}, '{{ addslashes($p->nama_pekerjaan) }}', '{{ $p->no_po }}', '{{ addslashes($p->client->nama ?? '') }}', '{{ addslashes($p->lokasi->nama_lokasi ?? '') }}', '{{ $p->tanggal }}', '{{ $p->status }}', '{{ $p->user_id }}')" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
...
<div id="modalAdd" class="fixed inset-0 z-50 hidden bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[calc(100vh-2rem)]">
        ...
        <form action="{{ route('admin.pekerjaan.store') }}" method="POST" class="p-5 sm:p-6 space-y-4 overflow-y-auto">
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1">Nomor PO (Opsional)</label>
                <input type="text" name="no_po" class="w-full border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none" placeholder="Contoh: PO-12345">
            </div>
            <div>
...
<div id="modalEdit" class="fixed inset-0 z-50 hidden bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[calc(100vh-2rem)]">
        ...
        <form id="formEdit" method="POST" class="p-5 sm:p-6 space-y-4 overflow-y-auto">
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1">Nomor PO (Opsional)</label>
                <input type="text" name="no_po" id="edit_no_po" class="w-full border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-brand-500 outline-none" placeholder="Contoh: PO-12345">
            </div>
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1">Client (Ketik nama client baru atau pilih)</label>
...
@push('scripts')
<script>
    function editProyek(id, nama, no_po, clientNama, lokasiNama, tanggal, status, userId) {
        document.getElementById('modalEdit').classList.remove('hidden');
        document.getElementById('formEdit').action = '/admin/pekerjaan/' + id;
        document.getElementById('edit_nama').value = nama;
        document.getElementById('edit_no_po').value = no_po;
